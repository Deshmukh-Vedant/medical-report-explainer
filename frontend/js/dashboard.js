...
async function viewReport(reportId) {
  ...
  if (detail.analysis) {
  ...
  } else if (detail.summary || detail.health_score != null || detail.risk_level) {
    const fallbackAnalysis = {
      patient_summary: detail.summary || "This report has a stored summary.",
      overall_health_summary: detail.summary || "This report has a stored summary.",
      key_findings: ["Stored summary available for this report."],
      health_score: detail.health_score || 0,
      risk_level: detail.risk_level || "Moderate",
      detected_tests: [],
      diet_advice: [],
      exercise_advice: [],
    ...
    renderAnalysis(fallbackAnalysis, reportId);
    document.getElementById("analysisResults")?.scrollIntoView({ behavior: "smooth" });
  } else {
