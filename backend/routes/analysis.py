...
def build_fallback_analysis(report: Report) -> ReportAnalysis:
    """Create a lightweight analysis payload from existing report fields."""
    summary_text = report.summary or "This report has a stored summary but no detailed analysis payload."
    return ReportAnalysis(
        patient_summary=summary_text,
        overall_health_summary=summary_text,
        key_findings=["Stored summary available for this report."],
        health_score=float(report.health_score or 0),
        risk_level=report.risk_level or "Moderate",
        detected_tests=[],
        diet_advice=[],
        exercise_advice=[],
        doctor_recommendation="Please review this report with your doctor.",
        red_flags=[],
        emergency_signs=[],
    )
...
def get_report(
    ...
    if report.raw_analysis_json:
        try:
            ...
            analysis = None
    if analysis is None and (report.summary or report.health_score is not None or report.risk_level):
